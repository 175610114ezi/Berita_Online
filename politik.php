<div class="articles">	
	<?php
		...
		$mulai  = ($hal_berita > 1) ? ($hal_berita * $perHalaman) - $perHalaman :0;
	    $artikel= mysqli_query($kon, "SELECT * FROM isi_berita WHERE id_kategori=2 ORDER BY tanggal DESC LIMIT $mulai, $perHalaman");

	    $tampil = mysqli_query($kon, "SELECT * FROM isi_berita WHERE id_kategori=2");//query untuk menghitung total jumlah keseluruhan tabel
	...
	<div align="center">
		<?php for($i=1; $i<=$semua_hal; $i++){?>
			<a href="index.php?menu=politik&halaman=<?php echo $i ?>"> <?php echo $i; ?></a>
		<?php }?>
	</div>
</div>
