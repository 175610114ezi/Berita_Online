...
<?php
		...
		$query = mysqli_query($kon, "UPDATE `isi_berita` SET `id_kategori` = '$id_kategori', `judul` = '$judul', `waktu` = now(), `gambar` = 'gambar/".$nama_file."', `isi` = '$isi' WHERE `isi_berita`.`id_berita` = '$id_berita' AND `isi_berita`.`id_kategori` = '$id_kategori'")or die(mysqli_error($kon));
